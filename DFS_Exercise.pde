...
/*
 TODO: 
    1. Try to understand how this Breadth-first Search (BFS) implementation works.
      ...
      A: nodes are represented as int, also as the index of neighbor array. edges are represented as the mapping between 
      index of neighbor array and it's value assoiate with index. 
      the purpose of visited list is to 1.keep track of all the nodes visited 2.avoid cyclic grach(which would cause infinite loop)
      the purpose of parent list is to reconstrct path once we find our goal.
      child node are gerring added to the fringe by the order they are save in neighbor list(FIFO)
    2. Convert this Breadth-first Search to a Depth-First Search.
       Which version BFS or DFS has a smaller maximum fring size?
       BFS has max of 4 but DFS has max of 3. DFS has a smaller maximum fring size. 
    3. Currently, the code sets up a graph which follows this tree-like structure: https://snipboard.io/6BhxRd.jpg
       ...
       when node 1 and 3 have the same child, we know that the graph is no longer a tree
       Breadth-first Search still find the optimal path
