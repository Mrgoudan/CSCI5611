...
void update(float dt){
  ...
  if (speed*dt > dir.length()-speed/2){
    vel = dir.times(0);
    pos = mousePos;
  }
else {
    dir.normalize();
    vel = dir.times(speed);
    // if(speed*dt > dir.length()&&vel.eql(oldVel)){
    //   println("in");
    //   vel = dir.times(0);
    //   pos = mousePos;
    // }
...
void draw(){
  ...
  stroke(0,0,0);
  fill(255,0,0);
  circle(pos.x, pos.y,r*2); //Question: Why is it radius*2 ?  A: to get the diameter of the circle since the thrid argument is the width and height of this circle. 
}
