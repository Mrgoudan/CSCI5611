...
TODO:
  ...
  DONE 2. Currently edge connections of any length are allowed. Update the connectNeighbors()
     ...
     A:the reason for this is because with shorter length, it's reduce the number of lines generated, which 
     would reduce computing power used. 
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  ...
  Vec2 tempPos = pointPos.minus(boxTopLeft);
  if(-1<tempPos.x&&tempPos.x<(boxW+1)&&-1<tempPos.y&&tempPos.y<(boxH+1)){
    return true;
  }
  return false;
}
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean insideAnyRec = pointInBox(boxTopLeft,boxW,boxH,randPos);
    while (insideAnyCircle||insideAnyRec){
      randPos = new Vec2(random(width),random(height));
      insideAnyCircle = pointInCircleList(circleCenters,circleRadii,randPos);
      insideAnyRec=pointInBox(boxTopLeft,boxW,boxH,randPos);
    }
    nodePos[i] = randPos;
  }
}
...
void connectNeighbors(){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      hitInfo recCheck = rayBoxIntersect(boxTopLeft, boxW, boxH, nodePos[i], dir, distBetween);

      if (!circleListCheck.hit&& distBetween < 200&&!recCheck.hit){
        neighbors[i].add(j);
      }
    }
  }
