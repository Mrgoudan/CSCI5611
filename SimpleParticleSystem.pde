...
//     horizontal motion (because you just add the vectors). Make sure your code solving Step 6 
//     does not have that issue, and that the red ball moves diagonally at the same speed it move horizontally or vertically.
...
void update(float dt){
  ...
  for (int i = 0; i < toGen; i++){
    ...
    vel[numParticles] = new Vec2(30+random(60),random(-200,-190)); 
    numParticles += 1;
  }
  ...
  if(obstacleVel.x!=0 && obstacleVel.y!=0) obstacleVel = new Vec2(obstacleVel.x/sqrt(2),obstacleVel.y/sqrt(2));
  ...
  for (int i = 0; i <  numParticles; i++){
    ...
    Vec2 acc = new Vec2(0,100*dt); //Gravity
    vel[i].add(acc);
    ...
    if (pos[i].distanceTo(spherePos) < (sphereRadius+r)){
      ...
      // velNormal.add(obstacleVel);
      // Vec2 velWithUser = velNormal.times(dot(velNormal,obstacleVel));
